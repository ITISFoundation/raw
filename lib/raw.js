...
!function(exports){
  ...
  raw.parser = function(delimiter) {
    ...
    function parser(string) {
      ...
      if (!parser.delimiter) parser.delimiter = detectDelimiter(string);
  ...
  raw.model = function() {
    ...
    model.autoDimensions = function(metadata) {
      if (metadata && metadata.length > 1) {
        dimensions.values()[0].value.push(metadata[0]);
        for(var i = 1; i < metadata.length; i++) {
          dimensions.values()[1].value.push(metadata[i]);
        }
      }
    }
